...
if (proc.status !== 0) {
console.error('[Error]: There was a problem fetching the diff:');
...
if (readme.length !== 1) {
	console.error('[Error]: PR changes files other than README!');
	process.exit(1);
}
...
if (chunks.length !== 1) {
	console.error('[Error]: Too many things changed in the README!');
	process.exit(1);
} 
...
if (added.length !== 1) {
	console.error('[Error]: Expected exactly one added line!');
	process.exit(1);
}
...
if (addedIndex === 0) {
	console.error('[Error]: Added line to beginning of README!');
	process.exit(1);
}

if (addedIndex === chunks[0].changes.length - 1) {
	console.error('[Error]: Added line to end of README -- please sign in alphabetical order by last name !');
	process.exit(1);
}
...
if (!beforeLine) {
	console.error('[Error]: Line before does not pass validation!');
	process.exit(1);
}

if (!afterLine) {
	console.error('[Error]: Line after does not pass validation!');
	process.exit(1);
}

if (!line) {
	console.error('[Error]: Invalid format for signature line!');
	process.exit(1);
}
...
if (beforeLastName > lastName || afterLastName < lastName) {
	console.error('[Error]: Signature does not appear to be alphabetical order!');
...
const isMergeResolution = ( pullRequester == 'igorkowalczyk' );

if (!( isOriginalPR || isMergeResolution )) {
	console.error('[Error]: Added username does not match pull requester!');
